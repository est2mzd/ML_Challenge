...
$Q_{\text{lon}}$は、自動運転車の縦方向の動作をキャプチャするために使用されます。これには、加速や減速、停止といった車両の進行方向に沿った挙動が含まれます。縦方向クエリは、学習可能なアンカーフリーのクエリであり、これにより自動運転車が柔軟に異なるシナリオに対応することができます。

縦方向クエリは、車両の速度、加速度、進行方向などの特徴を捉え、数式で表すと次のようになります：

```math
Q_{\text{lon}} \in \mathbb{R}^{N_L \times D}
```

ここで、$N_L$ は縦方向クエリの数を表し、$D$ はそれぞれのクエリの次元数です。

#### 変数の説明
- **縦方向クエリ $Q_{\text{lon}}$**: 車両の縦方向の動作（例：加速、減速、停止）を捉える学習可能なベクトル。
- **$N_L$**: 縦方向クエリの数。自動運転車が将来の動作を多様な形で計画するために複数のクエリが使用されます。
- **$D$**: 各縦方向クエリの埋め込みベクトルの次元。
...
### 3.4.2 Loss Calculation( 学習時の損失計算, [補足](./PLUT_Loss_Funcs.md) )
...
### 3.5.1.3  Leading Agents Dropout
