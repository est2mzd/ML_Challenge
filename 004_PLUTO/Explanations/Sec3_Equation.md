...
この式は、走行可能エリアの制約に違反した場合にペナルティを与えるものであり、またコストマップの変更によって衝突を避けるための処罰も可能です。

#### 実践における計算
この補助損失 $L_{\text{aux}}$ は、現代のディープラーニングフレームワークを用いてバッチごとに効率的に計算されます。このアプローチは、ESDFに基づく表現だけでなく、潜在的なフィールドなどの他のコスト表現にも適用可能であり、適切に設計された損失関数と組み合わせて使用されることが強調されています。
