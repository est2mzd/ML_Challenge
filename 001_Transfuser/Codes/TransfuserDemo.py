...
class TransFuser(nn.Module):
    def __init__(self):
        ...
        # 画像ブランチ
        #self.img_conv1 = nn.Conv2d(3, 72, kernel_size=3, stride=4, padding=1)  # [batch_size, 3, 160, 704] -> [batch_size, 72, 40, 176]
        ...
        # LiDARブランチ
        #self.lidar_conv1 = nn.Conv2d(3, 72, kernel_size=3, stride=4, padding=1)  # [batch_size, 3, 256, 256] -> [batch_size, 72, 64, 64]
    ...
    def fusion_block(self, img_features, lidar_features, positional_encoding, transformer, img_conv, lidar_conv):
        ...
        # デバッグ用のprint文
        #print(f"img_features_pos size: {img_features_pos.size()}")
        #print(f"lidar_features_pos size: {lidar_features_pos.size()}")
        ...
        #print(f"lidar_out size: {lidar_out.size()}")
    ...
    def loss(self, predicted_waypoints, ground_truth_waypoints):
        # 損失関数（平均二乗誤差）
        loss_fn = nn.MSELoss()
        loss = loss_fn(predicted_waypoints, ground_truth_waypoints)
        return loss
...
# フォワードパスの実行
predicted_waypoints = model(image_input, lidar_input)  # [batch_size, 4, 2]
...
# 損失の計算
loss = model.loss(predicted_waypoints, ground_truth_waypoints)  # スカラー値
